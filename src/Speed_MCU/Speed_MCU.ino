...
/**
 * Reads input speed on TX1/RX1 and TX2/RX2 serial lines, and forwards it onto the Speed topic of the DDS domain.
 */
...
XrceDomainParticipant participant(
    client, BASE_OBJECT_ID, "ACCParticipantLibrary::McuSpeed");

XrcePublisher publisher(
    client, BASE_OBJECT_ID, participant.id(), "Publisher");

XrceDataWriter writer(
    client, BASE_OBJECT_ID, publisher.id(), "SpeedWriter");
...
void wifi_connect()
{
    ArduinoOTA.setHostname("speedmcu");
...
void loop()
{
    ...
    if(current_millis - last_millis > MEASUREMENT_DELAY) {
        IntSensorData data;    
        uint32_t speed1 = serial_read(slave1);
        uint32_t speed2 = serial_read(slave2);

        // currently only forward speed1        
        data.payload = speed1;
